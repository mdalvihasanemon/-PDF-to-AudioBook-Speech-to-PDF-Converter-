...
def pdf_to_audio():
    #Creating a window 
    wn1 = tkinter.Tk() 
    wn1.title(" Alvi PDF to Audio converter")
    wn1.geometry('500x400')
    wn1.config(bg='snow3')
    
    start_pgNo = IntVar(wn1) #Variable to gold the starting page number
    end_pgNo = IntVar(wn1) #Variable to hold the ending page number
    
    Label(wn1, text='Alvi PDF to Audio converter',
...
def audio_to_pdf():
    #Creating a window
    wn2= tkinter.Tk() 
    wn2.title("AlviCoder Audio to PDF converter")
    wn2.geometry('500x400')
    wn2.config(bg='snow3')
    
    pdfPath = StringVar(wn2) #Variable to get the PDF path input
    
    Label(wn2, text='AlviCoder Audio to PDF converter',
      fg='black', font=('Courier', 15)).place(x=60, y=10)
...
wn = tkinter.Tk() 
wn.title("Alvi PDF to Audio and Audio to PDF converter")
wn.geometry('700x300')
wn.config(bg='LightBlue1')
  
Label(wn, text='Alvi PDF to Audio and Audio to PDF converter',
